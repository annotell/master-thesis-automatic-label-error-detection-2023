...
class BBoxHead(nn.Module):
    ...
    def get_bboxes(self,
        ...
        """
        # print(f'\n[bbox_head] def get_bboxes START: cls_score={cls_score[0].shape}, origin cls_score={cls_score}')
        # print(f'\n[bbox_head] def get_bboxes START: roi[0]={rois[0].shape}, origin roi={rois}')
        # print(f'\n[bbox_head] def get_bboxes START: bbox_pred={bbox_pred.shape}, origin bbox_pred={bbox_pred}')

        # print(f'SAVE [bbox_head]')
        # tmp_cls_score = cls_score[0].cpu().numpy().copy()
        # with open('bbox_head_cls_score.npy', 'wb') as f:
        #     np.save(f, tmp_cls_score)

        # tmp_rois = rois[0].cpu().numpy().copy()
        # with open('bbox_head_roi.npy', 'wb') as f:
        #     np.save(f, tmp_rois)

        # tmp_bbox_pred = bbox_pred.cpu().numpy().copy()
        # with open('bbox_head_bbox_pred.npy', 'wb') as f:
        #     np.save(f, tmp_bbox_pred)
        ...
        # print(f'\n[bbox_head] def get_bboxes after softmaxed: scores[0]={scores[0].shape}, softmaxed scores={scores}')
        
        # print(f'\n[bbox_head] def get_bboxes START: bbox_pred[0]={bbox_pred[0].shape}, origin bbox_pred={bbox_pred}')
        batch_mode = True
        if rois.ndim == 2:
        ...
        # print(f'\n[bbox_head] def get_bboxes, rois={rois[0].shape}, unsqueeze rois={rois}')
        ...
        if rescale and bboxes.size(-2) > 0:
            ...
            scale_factor = bboxes.new_tensor(scale_factor).unsqueeze(1).repeat(
                ...
                bboxes.size(-1) // 4)
            bboxes /= scale_factor
        # print(f'\n[bbox_head] def get_bboxes: bboxes={bboxes.shape}, scaled and before nms: bboxes={bboxes}')

        det_bboxes = []
        det_labels = []
        for (bbox, score) in zip(bboxes, scores):
            # each bbox has 11 scores
            # print(f'\n[bbox_head] zip(bboxes({bbox.shape}), scores({scores.shape})): bbox={bbox},\nscore={score}')
            ...
            # print(f'\n[bbox_head] def get_bboxes after multiclass_nms: det_bbox.shape={det_bbox.shape}, det_label.shape={det_label.shape}')
            
            det_bboxes.append(det_bbox)
            det_labels.append(det_label)
        # print(f'\n[bbox_head] def get_bboxes FINAL: det_bboxes={det_bboxes[0].shape}, det_bboxes={det_bboxes}')
        # print(f'\n[bbox_head] def get_bboxes FINAL: det_labels={det_labels[0].shape}, det_labels={det_labels}')
